...
// Siswa
    ...
    $this->pdf->generate('payout/payout_bill_pdf', $data, $siswa['student_full_name']);
...
// Siswa
    ...
    $this->pdf->generate('payout/payout_cetak_pdf', $data, 'Cetak_Struk_'.$siswa['student_full_name'].'_'.date('Y-m-d'));
...
// Siswa
    ...
    // tanggal
    $id[]=0;
    if (isset($f['c']) && ($f['c']) != NULL && $f['c'] != '') {

      $arr = $this->input->get('c');
      for ($i=0; $i<=$f['v']; $i++) 
      {
        if(isset($arr[$i])){
          $id[] = $arr[$i];
    ...
    $data['s_bl'] = $this->Bulan_model->get_total($cashback);
    $data['s_bb'] = $this->Bebas_pay_model->get($cashback);
    $data['log'] = $this->Log_trx_model->get(array('multiple_id' => $id));
    ...
    foreach ($data['log'] as $row) {
      $data['sum_m'] += $row['bulan_bill'];
    }

    $data['sum_b'] = 0;
    foreach ($data['log'] as $row) {
      $data['sum_b'] += $row['bebas_pay_bill'];
    }
    // endtotal
    ...
    $this->pdf->generate('payout/payout_cek_pdf', $data, 'Cetak_Struk_'.$siswa['student_full_name'].'_'.date('Y-m-d'));
...
// View data detail
    ...
    $this->Log_trx_model->delete_log(array(
      ...
      'bulan_id' => $id
        ...
        array(
          ...
          'log_module' => 'modul bulan',
          'log_action' => 'Hapus tagihan Bulan',
          'log_info' =>   'NamaSiswa :' . $student['student_full_name'] .',Nis:'. $student['student_nis'],
        )
      );
  ...
  function printPay($payment_id = NULL, $student_id =NULL, $id = NULL) {
    ...
    $this->pdf->generate('payout/payout_pdf', $data, $data['printpay']['student_full_name']);
  ...
  function printPayFree($payment_id = NULL, $student_id =NULL, $id = NULL) {
    ...
    $this->pdf->generate('payout/payout_free_pdf', $data, $data['printpay']['student_full_name']);
  ...
  function multiple() {
    ...
    if ($action == "printAll") {
      ...
      $this->pdf->generate('payout/payout_bulan_multiple_pdf', $data, 'Tagihan_Pembayaran_'.date('d-m-Y'));
    ...
    $this->Log_trx_model->delete_log(array(
      ...
      $this->Logs_model->add(
        array(
          'log_date' => date('Y-m-d H:i:s'),
          'user_id' => $this->session->userdata('uid'),
          'log_module' => 'modul bebas',
          'log_action' => 'Hapus Tagihan Bebas' ,
          'log_info' =>   'id siswa :' . $student_id ,
          )
        );
