...
class Report_set extends CI_Controller {
	...
	public function report()
	{
		...
		foreach ($data['bulan'] as $row) {
			...
			$objSheet->setCellValue('D'.$cell, $row['student_nis']);
			$objSheet->setCellValueExplicit('E'.$cell, $row['class_name'],PHPExcel_Cell_DataType::TYPE_STRING);
			$objSheet->setCellValue('F'.$cell, pretty_date($row['bulan_date_pay'], 'd/m/Y', FALSE));
			$objSheet->setCellValue('G'.$cell, $row['user_user_id']);
			$objSheet->setCellValue('H'.$cell, $row['bulan_bill']);
		...
		foreach ($data['free'] as $row) {
			...
			$objSheet->setCellValue('D'.$cell, $row['student_nis']);
			$objSheet->setCellValueExplicit('E'.$cell, $row['class_name'],PHPExcel_Cell_DataType::TYPE_STRING);
			$objSheet->setCellValue('F'.$cell, pretty_date($row['bebas_pay_input_date'], 'd/m/Y', FALSE));
			$objSheet->setCellValue('G'.$cell, $row['user_user_id']);
			$objSheet->setCellValue('H'.$cell, $row['bebas_pay_bill']);
			$objSheet->setCellValue('I'.$cell, ' ');
			$cell++;
			$no++;    
		}
		...
		foreach ($data['kredit'] as $row) {
			...
			$objSheet->setCellValue('F'.$cell, pretty_date($row['kredit_date'], 'd/m/Y', FALSE));
			$objSheet->setCellValue('G'.$cell, $row['user_user_id']);
		...
		foreach ($data['debit'] as $row) {
			...
			$objSheet->setCellValue('D'.$cell, '-');
			$objSheet->setCellValue('E'.$cell, '-');
			$objSheet->setCellValue('F'.$cell, pretty_date($row['debit_date'], 'd/m/Y', FALSE));
			$objSheet->setCellValue('G'.$cell, $row['user_user_id']);
		...
		$objXLS->getActiveSheet()->getColumnDimension('B')->setWidth(37);
		$objXLS->getActiveSheet()->getColumnDimension('C')->setWidth(23);
		...
		header('Content-Disposition: attachment;filename="KEUANGAN_PGRI1'.date('dmY').'.xls"'); 
